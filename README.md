...
A syntax checker for arithmetic expressions, built in pure Python with no
external libraries: a hand-written *lexer* (tokenizer) and a hand-written
*recursive-descent parser*.

It checks expressions like x + 3 * (y - 2) and reports whether they are
syntactically valid, with the exact error position when they are not.
...
Type tokens to toggle the token stream, tree to toggle the parse tree,
quit to exit.
...
This grammar is *left-recursive*, which a top-down (recursive-descent)
parser can’t handle directly — it would recurse forever. So the implemented
parser uses the standard left-recursion-elimination rewrite:
...
1. *Lexical analysis* (lexer.py): a single master regex scans the
   source left to right, classifying each chunk as NUMBER, IDENTIFIER,
   an operator, or a parenthesis. Whitespace is skipped. Anything that
   doesn’t match raises a LexerError with the exact character position.
1. *Syntax analysis* (syntax_parser.py): the Parser consumes the token
   list one token at a time (current() / advance()), calling itself
   recursively per the grammar above. If a token doesn’t fit what the
...
- Add assignment statements (x = expr) — turns this into a tiny
  language rather than just expression checking
- Generate an SLR/LR(0) parse table instead of recursive descent, to
  compare table-driven vs. hand-written parsing